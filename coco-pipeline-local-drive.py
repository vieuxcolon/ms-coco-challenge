...
# Verify directories
# ---------------------------
for path in [TRAIN_IMG_DIR, TRAIN_LABEL_DIR, TEST_IMG_DIR]:
    if not os.path.exists(path) or len(os.listdir(path)) == 0:
        raise FileNotFoundError(f"Directory missing or empty: {path}")
    else:
        print(f"✔ Found directory: {path} ({len(os.listdir(path))} files)")
